...
def test_crouter_add_path(tcrouter):
...
def test_crouter_rootnode_404(tcrouter):
    with pytest.raises(Exception):
        uri = '/badroot/somechild'
        vardict = dict()
        a = tcrouter.return_path_resource(uri,var_dict)()

def test_crouter_childnode_404(tcrouter):
    with pytest.raises(Exception):
        uri = '/rootnode/badchild'
        vardict = dict()
        a = tcrouter.return_path_resource(uri,var_dict)()

def test_crouter_add_varchild(tcrouter):
...
def test_crouter_get_var_resource(tcrouter):
    a = tcrouter.return_path_resource('/rootnode/test',dict())
    assert type(a) == setup_test_objects.TestHomeResource

def test_cadd_route_no_leading_slash(tcrouter):
